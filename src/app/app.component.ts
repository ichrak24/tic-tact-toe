...
export class AppComponent {
  ...
  generateGrid(gridSizeInput: HTMLInputElement) {
    ...
    if (size >= 3) {
      ...
      const cellWidth = (100 / this.gridSize) - 1; // Ajuster pour l'épaisseur des lignes
      const cellHeight = (100 / this.gridSize) - 1;
      let svgString = '';
  
      // Generate rectangles for the grid
      for (let i = 0; i < this.gridSize; i++) {
        for (let j = 0; j < this.gridSize; j++) {
          const x = j * (100 / this.gridSize);
          const y = i * (100 / this.gridSize);
          svgString += `<rect x="${x}%" y="${y}%" width="${cellWidth}%" height="${cellHeight}%" fill="green"></rect>`;
        }
      }
...
// Ajouter une méthode pour que l'ordinateur sélectionne une cellule disponible de manière aléatoire
getRandomEmptyCell(): { row: number, col: number } {
  ...
  if (emptyCells.length === 0) {
    return { row: -1, col: -1 }; // Aucune cellule vide disponible
  }
  ...
  // Modifier la méthode handleCellClick pour inclure le coup de l'ordinateur
  ...
  if (this.grid[row][col] === null) {
    ...
    if (winner) {
      ...
      if (row !== -1 && col !== -1) {
        ...
        this.currentPlayer = 1 - this.currentPlayer; // Passer au tour du joueur humain

        // Mettre à jour le SVG après le coup de l'ordinateur
        this.updateSVG(() => {});

        // Vérifier s'il y a un gagnant ou un match nul après le coup de l'ordinateur
        const computerWinner = this.checkWinner();
        if (computerWinner) {
          setTimeout(() => {
            if (this.grid.flat().filter(symbol => symbol === computerWinner).length >= 3) {
              this.gameResult = computerWinner === "X" ? 'Le joueur X a gagné !' : 'Le joueur O a gagné !';
              this.resetGame();
            }
          }, 0);
        } else if (this.isDraw()) {
